...
function Gathering:StatsPageOnUpdate(elapsed)
	...
	if (self.Ela > 10) then
		...
		Gathering:UpdateSessionTimeStat()
...
function Gathering:SetupStatsPage(page)
	...
	page.Stats.sessionlength = self:CreateStatLine(RightWidgets, format(L["Session Length: %s"], self:FormatFullTime(self.Seconds)))
...
function Gathering:UpdateSessionTimeStat()
	if (not self.Windows) then
		return
	end
	...
	if page.Stats.sessionlength then
		page.Stats.sessionlength.Text:SetText(format(L["Session Length: %s"], self:FormatFullTime(self.Seconds)))
	end
end
...
function Gathering:OnEnter()
	...
	if (TotalGathered > 0) then
		...
		Tooltip:AddDoubleLine(L["Session Length:"], self:FormatFullTime(self.Seconds), nil, nil, nil, 1, 1, 1)
